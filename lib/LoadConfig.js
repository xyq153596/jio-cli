...
module.exports = class LoadConfig {
  ...
  loadCreateConfig(target) {
    return target === "local"
      ...
      : {
          ...
          targetPath: path.resolve(target.path, target.name)
        };
  }
  /**
   * 得到相关如下格式的配置信息
   ...
   * @param {any} isLocal 是否更新本地模版
   * @return {Array}
   */
  loadUpdateConfig(select) {
    // 1.先根据配置确定更新本地还是用户项目
    const TEM_PATH = this.ctx.paths.getProjectInfo(select).path;
    //2.根据cli版本号确定需要更新的版本路径
    const TEM_CLI_VERSION = this.ctx.paths.getProjectInfo(select).version;
